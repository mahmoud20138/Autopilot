...
async function promptCLI() {
  const [openclaude, claudeCode, copilot, cursor, kilo] = await Promise.all([
    detector.isOpenClaudeInstalled(),
    detector.isClaudeCodeInstalled(),
    detector.isCopilotInstalled(),
    detector.isCursorInstalled(),
    detector.isKiloInstalled(),
  ]);
